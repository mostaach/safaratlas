...
export const Header: React.FC<HeaderProps> = ({ onOpenInquiryModal }) => {
  ...
  return (
    <header className="absolute top-0 left-0 right-0 z-50 bg-transparent border-none py-6 sm:py-8">
      <nav className="container-editorial flex items-center justify-between">

        {/* Left: Brand mark + Left-aligned CTA */}
        <div className="flex items-center gap-6 sm:gap-8">
          ...
          {/* Primary CTA — moved to left beside brand */}
          <div className="hidden lg:block">
            <a
              href={waUrl}
              target="_blank"
              rel="noreferrer"
              className="group inline-flex items-center gap-2.5 bg-[#C4A258] px-6 py-2.5 text-[10px] uppercase tracking-[0.2em] text-[#07192d] font-normal transition-all duration-300 hover:bg-[#d8bb78] hover:tracking-[0.24em] whitespace-nowrap rounded-none shadow-md"
        ...
        {/* Mobile controls: Book CTA + Hamburger */}
        <div className="flex items-center gap-2.5 md:hidden">
          <a
            href={waUrl}
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 bg-[#C4A258] px-3.5 py-2 text-[9px] uppercase tracking-[0.18em] text-[#07192d] font-normal rounded-none"
          >
            <span>Book</span>
