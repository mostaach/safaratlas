...
export const EscapesSection: React.FC<EscapesSectionProps> = ({
  ...
  return (
    <section
      id="escapes"
      className="py-14 lg:py-18 bg-[#07192d] text-[#f6f2ec] border-t border-white/10 relative overflow-hidden"
    >
      {/* Background Decorative Radial Glow */}
      <div className="absolute top-0 right-0 w-96 h-96 bg-[radial-gradient(#C4A258_1px,transparent_1px)] [background-size:24px_24px] opacity-15 pointer-events-none" />
      <div className="absolute -bottom-10 left-1/3 w-80 h-80 bg-[#C4A258]/5 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-7 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-5">
          <div className="space-y-2 max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#C4A258]/15 border border-[#C4A258]/30 text-[#C4A258] text-[11px] font-extrabold uppercase tracking-widest">
              <Sparkles className="w-3.5 h-3.5 text-[#C4A258]" />
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex flex-wrap gap-1.5 bg-[#0d2239] p-1.5 rounded-2xl border border-white/10 shadow-xs">
              {categories.map((cat) => (
                <button
                  key={cat}
                  ...
                  className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                    activeCategory === cat
                      ? "bg-[#C4A258] text-[#07192d] shadow-xs"
            ...
            <div className="hidden sm:flex items-center gap-2">
              <button
                ...
                className="w-10 h-10 rounded-xl bg-[#0d2239] border border-white/10 text-white hover:text-[#C4A258] hover:border-[#C4A258]/40 shadow-xs flex items-center justify-center transition-all cursor-pointer active:scale-95"
              >
        ...
        >
          {filteredEscapes.map((item) => (
            <div
              key={item.id}
              className="snap-start shrink-0 w-[300px] sm:w-[350px] lg:w-[365px] group bg-[#0d2239] rounded-2xl border border-white/10 overflow-hidden shadow-xl hover:border-[#C4A258]/40 transition-all duration-300 hover:-translate-y-1.5 flex flex-col"
            >
              ...
              {item.id === "escape-agafay-1d" || item.id === "escape-taghazout-3d" ? (
                ...
                >
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#07192d] via-black/30 to-transparent" />

                  {/* Badge */}
                  <span className="absolute top-3.5 left-3.5 px-3 py-1 rounded-lg bg-[#07192d]/90 backdrop-blur-md text-[#C4A258] text-[11px] font-extrabold uppercase tracking-wider border border-[#C4A258]/30 shadow-xs">
                    {item.badge}
                  </span>

                  {/* Location & Title */}
                  <div className="absolute bottom-3.5 left-3.5 right-3.5 text-white">
                    <span className="text-[11px] font-black text-[#C4A258] uppercase tracking-widest block drop-shadow-sm">
                      {item.location}
                    </span>
                    <h3 className="text-lg font-serif font-black tracking-tight drop-shadow-md text-white group-hover:text-[#C4A258] transition-colors line-clamp-1">
                      {item.title}
                    </h3>
                  </div>
                </Link>
              ...
              {/* Card Body */}
              <div className="p-5 flex flex-col flex-1 gap-4 bg-[#0d2239]">
                <p className="text-xs sm:text-sm text-white/70 leading-relaxed line-clamp-2">
                  {item.summary}
                </p>

                {/* Footer Section */}
                <div className="pt-3.5 border-t border-white/10 space-y-3.5 mt-auto">
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="text-base font-serif font-black text-white">
                        Price on WhatsApp
                      </span>
                      <span className="block text-[11px] font-semibold text-white/60 tracking-tight">
                    ...
                    {item.id === "escape-agafay-1d" ? (
                      <Link
                        href="/agafay"
                        className="cursor-pointer text-xs font-extrabold text-[#C4A258] hover:text-[#d8bb78] underline underline-offset-4 transition-colors"
                      >
                        View details →
                      </Link>
        ...
        <div className="rounded-2xl border border-white/10 bg-[#0d2239] p-5 sm:p-6 flex flex-col md:flex-row items-center justify-between gap-4 shadow-xl">
          ...
          <a
            href="#itineraries"
            className="shrink-0 px-6 py-3 rounded-xl bg-[#C4A258] hover:bg-[#d8bb78] text-[#07192d] text-xs font-black tracking-widest shadow-md transition-all hover:-translate-y-0.5"
          >
