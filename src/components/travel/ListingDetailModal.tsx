  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" role="presentation">
      <section aria-modal="true" aria-labelledby="listing-title" role="dialog" className="relative max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-3xl border border-slate-200 bg-cream shadow-2xl">
        ...
        <div className="relative h-64 sm:h-80"><Image src={images[imageIndex]} alt={business.name} fill sizes="(min-width: 768px) 768px, 100vw" className="object-cover" /><span className="absolute bottom-4 left-4 rounded-full bg-navy px-3 py-1 text-xs font-black text-accent">{business.priceRange}</span>{images.length > 1 && <div className="absolute bottom-4 right-4 flex gap-1.5 rounded-full bg-black/40 p-1.5">{images.map((_, index) => <button key={index} onClick={() => setImageIndex(index)} aria-label={`View image ${index + 1}`} className={`h-2 w-2 rounded-full ${index === imageIndex ? "w-5 bg-white" : "bg-white/60"}`} />)}</div>}</div>
