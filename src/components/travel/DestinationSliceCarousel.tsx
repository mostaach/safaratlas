...
function getSliceTint(id: string): string {
  const map: Record<string, string> = {
    marrakech:    "from-amber-950/80 via-rose-950/40 to-transparent",
    merzouga:     "from-amber-900/80 via-orange-950/40 to-transparent",
    taghazout:    "from-cyan-950/80 via-blue-950/40 to-transparent",
    chefchaouen:  "from-blue-950/80 via-indigo-950/40 to-transparent",
    fes:          "from-emerald-950/80 via-teal-950/40 to-transparent",
    essaouira:    "from-sky-950/80 via-slate-950/40 to-transparent",
  };
  return map[id] ?? "from-[#07192d]/80 via-[#07192d]/40 to-transparent";
}
        ...
        {destinations.map((dest, idx) => {
          ...
          return (
            ...
            >
              ...
              <div className={`absolute inset-0 bg-[#07192d] transition-opacity duration-500 ${isActive ? "opacity-0" : "opacity-40"}`} />
              ...
              >
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {/* Arabic accent */}
                    <span className="text-lg font-serif text-[#C4A258] px-2 py-0.5 rounded-lg bg-black/40 backdrop-blur-sm border border-white/10">
                      {dest.arabicName}
                    </span>
                ...
                <div className="text-white space-y-3">
                  ...
                  <div>
                    <h3 className="text-4xl font-serif font-bold tracking-tight drop-shadow-md leading-tight text-white">
                      {dest.name}
                    </h3>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <span className="text-[11px] text-white/70 font-medium">
                      Best season: <strong className="text-[#C4A258]">{dest.bestTime}</strong>
                    </span>

                    <button
                      onClick={e => { e.stopPropagation(); onSelect?.(dest); }}
                      className="relative overflow-hidden flex items-center gap-2 px-5 py-2.5 rounded-full bg-[#C4A258] hover:bg-[#d8bb78] text-[#07192d] text-xs font-bold tracking-widest shadow-lg transition-all cursor-pointer"
                    >
        ...
        {destinations.map((_, idx) => (
          <button
            ...
            className={`rounded-full transition-all duration-400 cursor-pointer ${
              idx === activeIdx
                ? "w-6 h-2 bg-[#C4A258]"
                : "w-2 h-2 bg-white/20 hover:bg-[#C4A258]/50"
            }`}
          />
        ))}
        <span className="ml-3 text-[11px] font-bold text-white/60">
        ...
        {destinations.map((dest, idx) => {
          ...
          return (
            ...
            >
              ...
              <div className={`absolute inset-0 bg-[#07192d] transition-opacity duration-500 ${isActive ? "opacity-0" : "opacity-30"}`} />
              ...
              <div className={`absolute inset-0 px-5 flex items-center justify-between transition-opacity duration-300 ${isActive ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
                ...
                <span className="text-xs font-bold text-[#C4A258]">Explore →</span>
              </div>
              ...
              <div className={`absolute inset-0 p-5 flex flex-col justify-between transition-opacity duration-500 ${isActive ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={e => { e.stopPropagation(); setSavedIdx(isSaved ? null : idx); }}
                      className={`p-2 rounded-full backdrop-blur-md border transition-all cursor-pointer ${isSaved ? "bg-[#C4A258] text-[#07192d] border-[#C4A258]" : "bg-black/40 text-white border-white/30"}`}
                    >
                ...
                <div className="text-white space-y-2">
                  ...
                  <h3 className="text-2xl font-serif font-bold tracking-tight text-white">{dest.name}</h3>
                  <p className="text-xs text-white/75 line-clamp-2">{dest.description}</p>
                  <div className="flex items-center justify-between pt-1">
                    <span className="text-[10px] text-white/70 font-medium">
                      Best: <strong className="text-[#C4A258]">{dest.bestTime}</strong>
                    </span>
                    <button
                      onClick={e => { e.stopPropagation(); onSelect?.(dest); }}
                      className="relative overflow-hidden flex items-center gap-1.5 px-4 py-2 rounded-full bg-[#C4A258] hover:bg-[#d8bb78] text-[#07192d] text-[11px] font-bold tracking-widest shadow-md transition-all cursor-pointer"
                    >
                      <div className="absolute inset-0 animate-shimmer pointer-events-none" />
